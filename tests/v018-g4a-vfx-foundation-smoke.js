...
assert.match(bootstrapSource,/js\/duel-vfx-v2\.js\?v=018-g4a/,'G4A VFX module must be in the public bootstrap chain');
...
assert.deepEqual(Array.from(api.DUEL_VFX_V2_FAMILIES),["physical","projectile","fire","frost","lightning"]);
...
assert.equal(api.isDuelVfxV2OwnedEvent({type:"heal",source:"heal"}),false,'unimplemented families must remain on the legacy fallback');
...
vfx.consume([
  ...
  {type:"heal",source:"heal",x:50,y:50}
]);
const status=vfx.getStatus();
assert.equal(status.active,3,'G4A router should accept only its owned family events');
