...
const visualBridgeTag='js/visual-bridge.js';
const ruleFeedbackTag='js/v013-rule-feedback.js';
const rareVfxTag='js/v016-rare-vfx.js?v=016dev-audit-r2';
for(const tag of [divineTag,r4Tag,endlessTag,codexTag,visualBridgeTag,ruleFeedbackTag,rareVfxTag])assert(index.includes(tag),`Public build must load ${tag}`);
...
assert(index.indexOf(endlessTag)<index.indexOf(codexTag),'Endless flow must register before Codex definitions are loaded');
assert(index.indexOf(codexTag)<index.indexOf(visualBridgeTag),'Visual Bridge must wrap the Codex preview function after Codex defines it');
assert(index.indexOf(visualBridgeTag)<index.indexOf(ruleFeedbackTag),'V0.13 rule feedback must remain layered after Visual Bridge');
assert(index.indexOf(ruleFeedbackTag)<index.indexOf(rareVfxTag),'V0.16 rare VFX must be the final preview/live wrapper so later visual layers cannot overwrite it');
