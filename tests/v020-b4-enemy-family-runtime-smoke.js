...
assert(runtime.includes('alliedDraws'),'allied runtime diagnostics missing');
assert(runtime.includes('familyDraws'),'per-family browser diagnostics missing');
for(const forbiddenRoot of ['root.ctx','root.player','root.state'])assert(!runtime.includes(forbiddenRoot),'classic-script lexical binding bug present: '+forbiddenRoot);
const v014Tag='<script src="js/v014-character-enemy-presentation.js"></script>';
const v020Marker='js/v020-enemy-presentation.js?';
const v014Pos=index.indexOf(v014Tag),v020Pos=index.indexOf(v020Marker);
assert(v014Pos>=0,'V0.14 enemy presentation runtime tag missing');
assert(v020Pos>=0,'V0.20 B4 enemy presentation runtime tag missing');
assert(v020Pos>v014Pos,'V0.20 B4 runtime must load after V0.14 presentation');
