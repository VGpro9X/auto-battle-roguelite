...
function resetRunState(){
  ...
  clearRunObjects();Object.assign(player,createInitialPlayer());for(const key of Object.keys(owned))delete owned[key];resetSkillEngine();resetMovementAI();refreshSkillBar();
}
...
function showLevelUp(isStarter=false){
  ...
  document.getElementById("skillPickDescription").textContent=isStarter?`Còn ${state.starterSelectionsRemaining} lượt chọn khởi đầu trong ${state.mode.label}.`:`1 lựa chọn nâng build · 1 kỹ năng mới · 1 lựa chọn thiên về synergy khi có thể.`;
  ...
  for(const key of picks){
    ...
    button.innerHTML=`<div class="icon">${skill.icon}</div><h3>${skill.name}</h3><div class="lvl">Cấp ${next}/${skill.max}${tags?` · ${tags}`:""}</div><div class="desc">${skill.desc(next)}</div>`;
...
function selectSkill(key,isStarter=false){
  owned[key]=(owned[key]||0)+1;skills[key].apply(owned[key]);onSkillSelectedEngine(key);refreshSkillBar();
...
function refreshSkillBar(){
  ...
  for(const key of keys){
    const element=document.createElement("div");element.className="skillMini";
    element.innerHTML=`<b>${getSkillDisplayIcon(key)} ${getSkillDisplayName(key)}</b><small>Lv.${owned[key]} · ${(skills[key].tags||[]).slice(0,2).join("/")}</small>`;bar.appendChild(element);
  }
  for(const synergy of Object.values(SYNERGIES)){
    if(!hasSynergy(synergy.id))continue;
    const element=document.createElement("div");element.className="skillMini";element.innerHTML=`<b>${synergy.icon} ${synergy.name}</b><small>SYNERGY</small>`;bar.appendChild(element);
  }
}
...
refreshSkillBar();renderSettings();showScreen("mainMenu");setGameUiVisible(false);document.title=`Auto Battle Roguelite ${GAME_VERSION}`;document.getElementById("version").textContent=`Auto Battle Roguelite ${GAME_VERSION}`;
