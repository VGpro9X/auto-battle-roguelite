(()=>{
  ...
  function qualityProfile(options={}){const legacy=typeof root.getDuelVisualQuality==="function"?root.getDuelVisualQuality():null,requested=String(options.quality||runtimeQuality()||root.DUEL_RENDERER_V3_QUALITY||"auto").toLowerCase(),quality=requested==="low"?"low":requested==="full"?"full":requested==="balanced"?"balanced":legacy?.id==="constrained"?"low":"balanced",reduced=typeof options.reducedMotion==="boolean"?options.reducedMotion:Boolean(legacy?.reducedMotion);return quality==="full"?{quality,reduced,limit:48,detail:1,alpha:1}:quality==="low"?{quality,reduced,limit:16,detail:.48,alpha:.66}:{quality,reduced,limit:30,detail:.72,alpha:.84};}
